...
# CUDA_VISIBLE_DEVICES=$VLLM_GPU_ID vllm serve "$MODEL_PATH" --host 0.0.0.0 --port "$VLLM_PORT" --gpu-memory-utilization 0.65 --max-num-seqs 16 --disable-log-requests --dtype float16 &
CUDA_VISIBLE_DEVICES=$VLLM_GPU_ID vllm serve "$MODEL_PATH" --host 0.0.0.0 --port "$VLLM_PORT" --dtype float16 --gpu-memory-utilization 0.5 --max-num-seqs 1 --disable-log-requests &

...
def split_markdown_by_headings(
    ...
    """
    if len(headings) == 0 or len(adjusted_headings) == 0:
...
def build_heading_context(
    ...
    for idx, line in enumerate(lines):
        ...
        if stripped in heading_set:
            ...
            heading_positions.append((idx, stripped, level))

    blocks = []

    for pos_idx, (line_idx, heading, level) in enumerate(heading_positions):
        end_idx = len(lines)
        for next_line_idx, _, next_level in heading_positions[pos_idx + 1 :]:
            if next_level <= level:
...
class Document:
    ...
    def _parse_chunk(
        ...
        try:
            ...
            section = Section.from_dict(section, markdown_content=section_markdown)
            for media in section.iter_medias():
    ...
    def from_markdown(
        ...
        heading_context = build_heading_context(markdown_content, headings)
        adjusted_headings = language_model(
            HEADING_EXTRACT_PROMPT.render(heading_context=heading_context),
            return_json=True,
        )
        ...
        for section in self.sections:
            ...
            for subsection in section.subsections:
                ...
                for media in subsection.medias:
                    overview += f"\t\tMedia ({media.media_type}): {media.caption}\n"
                overview += "\n"
        return overview
...
class OutlineItem:
    ...
    async def check_images_async(
        ...
        for idx, image in enumerate(self.images):
            ...
            if cosine_similarity(embedding, image_embeddings[similar]) > sim_bound:
                if similar >= len(doc_images):
                    similar = min(similar, len(doc_images) - 1)
                self.images[idx] = doc_images[similar].caption
            else:
                logger.warning(
                    f"image not found: {image}, available images: {[image.caption for image in doc_images]}.",

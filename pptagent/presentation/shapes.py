...
def _configure_movie_playback(slide: PPTXSlide, shape_id: int) -> None:
    timing = slide._element.find(qn("p:timing"))
    if timing is None:
        return

    video_nodes = timing.findall(".//p:video", namespaces={"p": PML_NS})
    target_video = None
    for video in video_nodes:
        sp_tgt = video.find(".//p:spTgt", namespaces={"p": PML_NS})
        if sp_tgt is not None and sp_tgt.get("spid") == str(shape_id):
            target_video = video
            break
    if target_video is None:
        return

    ctn = target_video.find(".//p:cTn", namespaces={"p": PML_NS})
    if ctn is None:
        return
    ctn.set("repeatCount", "indefinite")

    st_cond_lst = target_video.find(".//p:stCondLst", namespaces={"p": PML_NS})
    if st_cond_lst is None:
        st_cond_lst = parse_xml(f'<p:stCondLst xmlns:p="{PML_NS}"/>')
        ctn.insert(0, st_cond_lst)
    else:
        for child in list(st_cond_lst):
            st_cond_lst.remove(child)

    st_cond_lst.append(
        parse_xml(f'<p:cond xmlns:p="{PML_NS}" evt="onBegin" delay="0"/>')
    )
...
class Picture(ShapeElement):
    ...
    def build(self, slide: PPTXSlide) -> PPTXPicture:
        ...
        if self.is_video:
            ...
            _configure_movie_playback(slide, shape.shape_id)
            return shape

...
def resolve_soffice_command() -> list[str] | None:
    candidates = [
        which("soffice"),
        which("soffice.exe"),
        which("soffice.cmd"),
...
SOFFICE_COMMAND = resolve_soffice_command()
...
def ppt_to_images(file: str, output_dir: str):
    ...
    if SOFFICE_COMMAND is None:
        raise FileNotFoundError("Could not resolve a runnable soffice command")
    if pexists(output_dir):
        logger.warning(f"ppt2images: {output_dir} already exists")
    os.makedirs(output_dir, exist_ok=True)
    with tempfile.TemporaryDirectory() as temp_dir:
        command_list = SOFFICE_COMMAND + [
            "--headless",
            "--convert-to",
            "pdf",
            file,

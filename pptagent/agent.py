...
from pptagent.utils import get_json_from_response, get_logger, package_join
...
class AsyncAgent(Agent):
    ...
    async def retry(self, feedback: str, traceback: str, turn_id: int, error_idx: int):
        ...
        logger.info(
            "[agent:retry:start] role=%s model=%s turn=%s retry=%s prompt_chars=%s history_messages=%s",
            self.name,
            self.model,
            turn_id,
            error_idx,
        ...
        try:
            ...
            logger.exception(
                "[agent:retry:error] role=%s model=%s turn=%s retry=%s elapsed=%.2fs",
                self.name,
                self.model,
                turn_id,
                error_idx,
            ...
            raise
        logger.info(
            "[agent:retry:done] role=%s model=%s turn=%s retry=%s elapsed=%.2fs response_chars=%s",
            self.name,
            self.model,
            turn_id,
            error_idx,
    ...
    async def __call__(
        ...
        **jinja_args,
        ...
        started = perf_counter()
        logger.info(
            "[agent:start] role=%s model=%s turn=%s prompt_chars=%s history_messages=%s images=%s args=%s",
            self.name,
            self.model,
            turn_id,
            len(prompt),
            len(history_msg),
            0 if images is None else len(images),
            ",".join(sorted(jinja_args.keys())),
        )
        try:
            response, message = await self.llm(
                prompt,
            ...
            logger.exception(
                "[agent:error] role=%s model=%s turn=%s elapsed=%.2fs",
                self.name,
                self.model,
                turn_id,
                perf_counter() - started,
            ...
            raise
        logger.info(
            "[agent:done] role=%s model=%s turn=%s elapsed=%.2fs response_chars=%s",
            self.name,
            self.model,
            turn_id,
            perf_counter() - started,

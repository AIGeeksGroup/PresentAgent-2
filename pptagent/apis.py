...
GIF_REGEX = re.compile(r".*\.gif$", re.IGNORECASE)
GIF_AS_VIDEO = os.environ.get("PPTAGENT_GIF_AS_VIDEO", "1").lower() in {
    "1",
    "true",
    "yes",
}
...
def replace_image(slide: SlidePage, doc: Document, img_id: int, image_path: str):
    ...
    """
    if GIF_REGEX.match(image_path) and GIF_AS_VIDEO:
        try:
            image_path = _convert_gif_to_mp4(image_path)
        except SlideEditError as exc:
            logger.warning(
                "GIF conversion unavailable, embedding the original GIF as an image: %s",
                exc,
            )
    ...
    _replace_picture_asset(shape, image_path)


def _replace_picture_asset(shape: Picture, image_path: str):
...
def _convert_gif_to_mp4(gif_path: str) -> str:
    """
    Convert an animated GIF to an MP4 file that PowerPoint can embed reliably.
    """
    if not os.path.exists(gif_path):
        raise SlideEditError(f"The GIF {gif_path} does not exist.")
    mp4_path = os.path.splitext(gif_path)[0] + ".pptagent.mp4"
    ...
    if ffmpeg is None:
        raise SlideEditError(
            "Animated GIF needs ffmpeg to be converted to MP4 before embedding. "
            "Please install ffmpeg or provide an MP4/WebM/MOV/M4V asset."
        )
    ...
    if result.returncode != 0 or not os.path.exists(mp4_path):
        raise SlideEditError(
            "Failed to convert GIF to MP4 with ffmpeg: "
...
def replace_video(slide: SlidePage, img_id: int, video_path: str):
    ...
    """
    if GIF_REGEX.match(video_path) and not GIF_AS_VIDEO:
        shape = element_index(slide, img_id)
        if not isinstance(shape, Picture):
            raise SlideEditError(
                f"The element {shape.shape_idx} of slide {slide.slide_idx} is not a Picture."
            )
        _replace_picture_asset(shape, video_path)
        return
    if GIF_REGEX.match(video_path):
        try:
            video_path = _convert_gif_to_mp4(video_path)
        except SlideEditError as exc:
            raise SlideEditError(
                f"GIF must be embedded as video, but conversion failed for {video_path}: {exc}"
            ) from exc
    ...
    if not VIDEO_REGEX.match(video_path):
        _replace_picture_asset(shape, video_path)
        return
